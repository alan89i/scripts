...
$result = ibase_query($dbh,"select  a.codigo_laudo, a.nome_laudo, texto_laudo from l_laudo ");
...
//    var_dump(gettype($res->LAUDO));
    if($res->LAUDO !='') {
        $blob_data = ibase_blob_info($res->LAUDO);
        $blob_hndl = ibase_blob_open($res->LAUDO);
...
//    $txt = str_replace("'", '@', $txt);
    ...
    $sql = "insert into ultrassonografiabotafogoeireli.laudos_feegow (laudo, matricula, titulo, digitado_em, codigo_laudo, codigo_exame, nome_cliente, digitado_por, codigo_paciente) values('$txt', '{$res->MATRICULA}', '{$res->TITULO}', '{$res->DIGITADO_EM}', '{$res->CODIGO_LAUDO}', '{$res->CODIGO_EXAME}', '{$res->NOME_CLIENTE}', '{$res->DIGITADO_POR}', '{$res->CODIGO_PACIENTE}')";
